...
- Script-to-scene planning with an exact target image count
...
4. On a few scene cards, select **Offline test** as the provider.
...
5. Click **Generate all**. The queue can be paused, resumed, or safely restarted.
...
7. Export locally. Caption burning can be disabled to produce a clean video;
   `captions.srt` is always written in the project folder.
...
Automatic word-level alignment in the UI is the first roadmap upgrade. The
